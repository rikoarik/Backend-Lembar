...
export async function registerAiPromptRoutes(
  ...
  app.get('/v1/admin/prompts/:id', { preHandler: [auth, superadmin] }, async (request, reply) => {
    ...
    const evalRes = await pool.query(
      'SELECT * FROM ai_prompt_eval_cases WHERE prompt_id = $1 ORDER BY created_at',
      [id],
    );
    ...
    return reply.status(200).send({
      data: {
        ...
        evalCases: evalRes.rows.map((e: any) => ({
          ...
          inputSignals: e.input_signals,
          expectedOutput: e.expected_output,
          validateRules: e.validate_rules,
  ...
  app.post('/v1/admin/prompts', { preHandler: [auth, superadmin] }, async (request, reply) => {
    ...
    const [created] = await db
      ...
      .values({
        ...
        contextWindow: body.context_window ?? 'default',
    ...
    async (request, reply) => {
      ...
      if (!body?.status || !['active', 'draft'].includes(body.status))
        return reply
          .status(400)
          .send({
            error: { code: 'VALIDATION_FAILED', message: 'status must be active or draft' },
          });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!body?.prompt_text)
        return reply
          .status(400)
          .send({ error: { code: 'VALIDATION_FAILED', message: 'prompt_text required' } });
      ...
      const currentVer =
        parseInt(String((promptRes.rows[0] as any).version).replace(/^v/, ''), 10) || 0;
      const nextVersion = currentVer + 1;
      const user = request.jwtUser!;

      await pool.query(
        'UPDATE ai_prompt_versions SET status = $1 WHERE prompt_id = $2 AND status = $3',
        ['archived', id, 'active'],
      );
      await pool.query(
      ...
      await db
        .update(adminPrompts)
        .set({ version: String(nextVersion), updatedAt: new Date() })
        .where(eq(adminPrompts.id, id));
  ...
  app.patch(
    '/v1/admin/prompts/:id/versions/:version/activate',
    { preHandler: [auth, superadmin] },
    async (request, reply) => {
      ...
      if (isNaN(ver))
        return reply
          .status(400)
          .send({ error: { code: 'VALIDATION_FAILED', message: 'version must be a number' } });
      ...
      const vRes = await pool.query(
        'SELECT id FROM ai_prompt_versions WHERE prompt_id = $1 AND version = $2',
        [id, ver],
      );
      if (!vRes.rows[0])
        return reply
          .status(404)
          .send({ error: { code: 'RESOURCE_NOT_FOUND', message: 'Version not found' } });
      ...
      await pool.query(
        'UPDATE ai_prompt_versions SET status = $1 WHERE prompt_id = $2 AND version = $3',
        ['active', id, ver],
      );
      await db
        .update(adminPrompts)
        .set({ activeVersion: String(ver), updatedAt: new Date() })
        .where(eq(adminPrompts.id, id));
  ...
  // ── List eval cases ────────────────────────────────
  app.get(
    '/v1/admin/prompts/:id/eval-cases',
    { preHandler: [auth, superadmin] },
    async (request, reply) => {
      const { id } = request.params as { id: string };
      const pool = getPool(db);
      ...
      return reply.status(200).send({
        data: result.rows.map((r: any) => ({
          ...
          inputSignals: r.input_signals,
          expectedOutput: r.expected_output,
          validateRules: r.validate_rules,
          createdAt: r.created_at,
        })),
      });
    },
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!body?.label)
        return reply
          .status(400)
          .send({ error: { code: 'VALIDATION_FAILED', message: 'label required' } });
      ...
      const promptRes = await pool.query('SELECT active_version FROM admin_prompts WHERE id = $1', [
        id,
      ]);
      if (!promptRes.rows[0])
        return reply
      ...
      await pool.query(
        ...
        [
          ...
          pv,
          body.label,
          JSON.stringify(body.input_signals ?? {}),
          JSON.stringify(body.expected_output ?? {}),
          JSON.stringify(body.validate_rules ?? {}),
        ],
      );
  ...
  app.get('/v1/admin/schemas', { preHandler: [auth, superadmin] }, async (_request, reply) => {
    ...
    const result = await pool.query(
      'SELECT id, name, schema_type, version, created_at FROM ai_prompt_schemas ORDER BY created_at',
    );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!body?.rating || body.rating < 1 || body.rating > 5)
        return reply
          .status(400)
          .send({ error: { code: 'VALIDATION_FAILED', message: 'rating must be 1-5' } });
  ...
  // ── Learning signals ───────────────────────────────
  // Aggregate the feedback table directly; no ai_learning_signals table exists.
  app.get(
    ...
    async (_request, reply) => {
      ...
      const result = await pool.query(`
      SELECT prompt_template_id, pattern, frequency, avg_rating
      FROM (
