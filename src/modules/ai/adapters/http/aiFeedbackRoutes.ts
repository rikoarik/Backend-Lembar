...
export async function registerAiFeedbackRoutes(
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!user)
        return reply
          .status(401)
          .send({ error: { code: 'AUTH_REQUIRED', message: 'Login required' } });
      const workspaceId = (user as any).workspaceId as string | undefined;
      if (!workspaceId) {
        return reply
          .status(400)
          .send({ error: { code: 'WORKSPACE_REQUIRED', message: 'Workspace context missing' } });
      }
      ...
      if (
        ...
        body.promptTemplateId.length === 0
      ) {
        return reply
          .status(400)
          .send({ error: { code: 'VALIDATION_FAILED', message: 'promptTemplateId is required' } });
      }
      const rating = Number(body.rating);
      if (!Number.isInteger(rating) || rating < 1 || rating > 5) {
        return reply
          .status(400)
          .send({ error: { code: 'VALIDATION_FAILED', message: 'rating must be integer 1..5' } });
      }
      ...
      // Attribute to the real prompt template when the client supplied a jobId:
      // never trust a client-provided generic label like 'assessment_output'.
      let promptTemplateId = body.promptTemplateId.trim();
      if (body.jobId) {
        const audit = await pool
          .query(
            `SELECT prompt_template_id FROM ai_jobs_audit
            WHERE job_id = $1 AND workspace_id = $2
            ORDER BY created_at DESC LIMIT 1`,
            [body.jobId, workspaceId],
          )
          .catch(() => ({ rows: [] }));
        const resolved = (audit.rows[0] as { prompt_template_id?: string } | undefined)
          ?.prompt_template_id;
        if (resolved) promptTemplateId = resolved;
      }
      if (promptTemplateId.startsWith('smoke.') || promptTemplateId === 'assessment_output') {
        return reply.status(400).send({
          error: {
            code: 'VALIDATION_FAILED',
            message: 'promptTemplateId generik tidak dapat menerima feedback',
          },
        });
      }
  ...
  // Aggregated live from ai_feedback (scoped to this workspace) — there is no ai_learning_signals table.
  app.get('/v1/ai/learning-signals', { preHandler: [auth] }, async (request, reply) => {
    const user = request.jwtUser;
    const workspaceId = (user as any)?.workspaceId as string | undefined;
    const pool = getPool(db);
    if (!pool || !workspaceId) return reply.status(200).send({ data: [] });
    const result = await pool.query(
      `SELECT prompt_template_id, pattern, frequency, avg_rating
           FROM (
             ...
             FROM ai_feedback
             WHERE workspace_id = $1 AND comment IS NOT NULL AND comment != ''
